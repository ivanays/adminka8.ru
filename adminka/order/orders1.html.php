...
<html lang="ru">
  ...
  <body>
	...
	<fieldset>
      ...
      <?php 	if (isset($orders1) and $orders1 != '') {?>
      <?php foreach ($orders1 as $order1): ?>
            <li>
			 <form action="" method="post">
			 <div>
              <?php echo '№ '; ?>			
                  <?php htmlout($order1['id']); ?>
              <?php //htmlout($order['price']); ?>				   
              <?php htmlout($order1['plus']); ?>				   
              <?php htmlout($order1['sale']); ?>				   
              <?php// htmlout($order['all_price']); ?>				   
              <?php htmlout(date('d.m.Y H:i:s', $order1['time_in'])); ?>
              <?php htmlout(date('d.m.Y H:i:s', $order1['time_out'])); ?>
              <?php htmlout($order1['description']); ?>				   
              <?php htmlout($order1['state']); ?>				   				              
