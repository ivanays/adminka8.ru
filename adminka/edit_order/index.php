...
if (isset($_POST['action_user']) and $_POST['action_user'] == 'РЕДАКТИРОВАТЬ')
{
  ...
  if (isset($_POST['ouid']) && $_POST['ouid'] != '' && is_numeric($_POST['ouid']))
  {
	  $oid = $_POST['ouid'] ?? false;
...
if (isset($_GET['editgroupform']))
{
  ...
  if (isset($_POST['password']) && $_POST['password'] != '')
  {
	...
	$pageTitle = 'ДАННЫЕ КЛ-НТА №' . $uid . ' ' . 'ОТРЕДАКТИРОВАНЫ.';  

    include 'form_ready.html.php';  
...
if (isset($_POST['action_user']) and $_POST['action_user'] == 'УДАЛИТЬ ИЗ ГРУППЫ')
{
   ...
   if (isset($_POST['ugid']) && $_POST['ugid'] != '' && is_numeric($_POST['ugid']))
  {
    ...
    exit();
  	...
  	$pageTitle = 'УДАЛЕНИЕ ИЗ ГРУППЫ КЛ-НТА №' . $ugid;
...
if (isset($_GET['addformgroup']))
{
    ...
    if (isset($_POST['number1']) && $_POST['number1'] != '' && is_numeric($_POST['number1']) && isset($_POST['number2']) && $_POST['number2'] != '' && is_numeric($_POST['number2']) && isset($_POST['number3']) && $_POST['number3'] != '' && is_numeric($_POST['number3']) && isset($_POST['number4']) && $_POST['number4'] != '' && is_numeric($_POST['number4']))
  {
    ...
    if ((isset($_POST['guid']) && $_POST['guid'] != '' && is_numeric($_POST['guid'])) || (isset($_POST['surname']) && $_POST['surname'] != '') || (isset($_POST['firstname']) && $_POST['firstname'] != '') || (isset($_POST['middle_name']) && $_POST['middle_name'] != '') || (isset($_POST['operator']) && $_POST['operator'] != '') || (isset($_POST['region']) && $_POST['region'] != '') || (isset($_POST['status']) && $_POST['status'] != ''))
	{
	   $guid = $_POST['guid'] ?? false;
	   $surname = $_POST['surname'] ?? false;
	   $firstname = $_POST['firstname'] ?? false;
  ...
  catch (PDOException $e)
  {
	...
	$pageTitle = 'КЛ-НТ №' . $uid . ' ' . 'В ГРУППУ ДОБАВЛЕН.';  
...
if (isset($_POST['action']) and $_POST['action'] == 'ДОБАВИТЬ НОВУЮ УСЛУГУ')
{
  ...
  catch (PDOException $e)
  {
	...
	$pageTitle = 'ИЗДЕЛИЕ №' . $point_product_id . '. ' . ' ' . 'ДОБАВЛЕНА УСЛУГА №' . $poid . '.';   
...
if (isset($_POST['action_point']) and $_POST['action_point'] == 'РЕДАКТИРОВАТЬ')
{
  ...
  include $_SERVER['DOCUMENT_ROOT'] . '/includes/db.inc.php';   
    ...
    $pageTitle = 'СЧЁТ №' . $point_oid 	. '.' . ' ' . 'ИЗДЕЛИЕ №' . $product_id . '.' . ' ' . 'УСЛУГА №' . $point_id . '.' . ' ' . 'Отредактировать данные.' ;
...
if (isset($_GET['editformpoint']))
{
  ...
  catch (PDOException $e)
  {
    ...
    $pageTitle = ' ИЗДЕЛИЕ №' . $prid . '.' . ' ' . 'УСЛУГА №' . $poid . '.' . ' ' . 'ДАННЫЕ ОТРЕДАКТИРОВАНЫ.'  ;
...
if (isset($_POST['action']) and $_POST['action'] == 'ДОБАВИТЬ НОВОЕ ИЗДЕЛИЕ')
{
  ...
  $prid = $pdo->lastInsertId();  
    ...
    $pageTitle = ' ИЗДЕЛИЕ №' . $prid .  ' ' . 'ДОБАВЛЕНО.'  ;
...
if (isset($_POST['action_product']) and $_POST['action_product'] == 'РЕДАКТИРОВАТЬ')
{
  ...
  include $_SERVER['DOCUMENT_ROOT'] . '/includes/db.inc.php';
    ...
    $pageTitle = 'Редактировать данные.';
...
if (isset($_GET['editformproduct']))
{
  ...
  catch (PDOException $e)
  {
    ...
    $pageTitle = ' ИЗДЕЛИЕ №' . $prid . '.' . ' ДАННЫЕ ОТРЕДАКТИРОВАНЫ.';  
...
if (isset($_POST['action']) and $_POST['action'] == 'ПРОВЕСТИ ТРАНЗАКЦИЮ')
{
  ...
  catch (PDOException $e)
  {
	...
	$caid = $pdo->lastInsertId();
	
    $pageTitle = ' ТРАНЗАКЦИЯ №' . $caid . ' ' . 'ПРОВЕДЕНА.';  
...
if (isset($_GET['editformcash']))
{
  ...
  catch (PDOException $e)
  {
    ...
    $pageTitle = ' ТРАНЗАКЦИЯ №' . $caid . ' ' . 'ОТРЕДАКТИРОВАНА.';  
...
if (isset($_POST['action']) and $_POST['action'] == 'РЕДАКТИРОВАТЬ СЧЁТ')
{ 
        $oid = '';
	$order_plus_add = '';
	$order_sale_add = '';
  ...
  try
  {
    ...
    $s->bindValue(':time_out', $order_time_out);
  ...
  } elseif ($order_state == 'ОТКРЫТ')

  {

     	$order_time_out = '0000000000';
...
if (isset($_POST['action_order']) and $_POST['action_order'] == 'УДАЛИТЬ СЧЁТ В КОРЗИНУ')
{ 
    $oid = '';
	$o_trash = '';
	$pageTitle = '';
  ...
  catch (PDOException $e)
  {
    ...
    $pageTitle = ' ' . ' ' . 'УДАЛЁН В КОРЗИНУ.';  
 
	include 'form_ready_orders.html.php'; 
...
if (isset($_POST['action_order']) and $_POST['action_order'] == 'ВОССТАНОВЛЕНИЕ')
{ 
    $oid = '';
	$o_trash = '';
	$pageTitle = '';
...
if (isset($_POST['action_order']) and $_POST['action_order'] == 'ПОЛНОЕ УДАЛЕНИЕ')
{ 
    $oid = '';
	$pageTitle = '';
  ...
  try
  {
    $sql = 'DELETE FROM points WHERE order_id = :order_id';
    $s = $pdo->prepare($sql);
    $s->bindValue(':order_id', $oid);
    $s->execute();
  }
  ...
  {
    $error = 'Ошибка удаления данных1.' . $e->getMessage();
    include 'error.html.php';
    exit();
  }
  ...
  try
  {
    $sql = 'DELETE FROM products WHERE order_id = :order_id';
    $s = $pdo->prepare($sql);
    $s->bindValue(':order_id', $oid);
    $s->execute();
  }
  ...
  {
    $error = 'Ошибка удаления данных2.' . $e->getMessage();
    include 'error.html.php';
    exit();
  }
  ...
  try
  {
    $sql = 'DELETE FROM cash WHERE order_id = :order_id';
    $s = $pdo->prepare($sql);
    $s->bindValue(':order_id', $oid);
    $s->execute();
  }
  ...
  {
    $error = 'Ошибка удаления данных3.' . $e->getMessage();
    include 'error.html.php';
    exit();
  }  
  ...
  {
    $error = 'Ошибка удаления данных4.' . $e->getMessage();
    include 'error.html.php';
    exit();
  }	
    ...
    $pageTitle = ' ' . ' ' . 'БЫЛ ПОЛНОСТЬЮ УДАЛЁН.';  
...
if (isset($_POST['action_product']) && $_POST['action_product'] == 'ДА')
{
  ...
  catch (PDOException $e)
  {
	...
	$pageTitle = ' ИЗДЕЛИЕ №' . $prid . ' ' . 'УДАЛЕНО.';
...
if (isset($_POST['action_point']) && $_POST['action_point'] == 'ДА')
{
	...
	if ((isset($_POST['poid']) && $_POST['poid'] != '' && is_numeric($_POST['poid'])) && (isset($_POST['oid']) && $_POST['oid'] != '' && is_numeric($_POST['oid']))   && (isset($_POST['prid']) && $_POST['prid'] != '' && is_numeric($_POST['prid'])))
	{
		$poid = $_POST['poid'] ?? false;
		$oid = $_POST['oid'] ?? false;
		$prid = $_POST['prid'] ?? false;		
  ...
  catch (PDOException $e)
  {
	...
	$pageTitle = ' ИЗДЕЛИЕ №' . $prid . ' .' . ' ' . 'УСЛУГА №' . $poid . '	УДАЛЕНА.';
...
if (isset($_POST['action_cash']) && $_POST['action_cash'] == 'ДА')
{
  ...
  catch (PDOException $e)
  {
    ...
    $pageTitle = ' ТРАНЗАКЦИЯ №' . $caid . ' ' . 'УДАЛЕНА.';  
...
if (isset($_POST['action_order']) && $_POST['action_order'] == 'ПРОСМОТР СЧЁТА')
{
 ...
 foreach ($s as $row)
{
  $useres[] = array('uid' => $row['uid'], 'number' => '8', 'number1' => substr($row['phone'], 1, 3), 'number2' => substr($row['phone'], 4, 3), 'number3' => substr($row['phone'], 7, 2), 'number4' => substr($row['phone'], 9, 2), 'surname' => $row['surname'], 'firstname' => $row['firstname'], 'middle_name' => $row['middle_name'], 'time_reg' => date('d.m.Y H:i:s', $row['time_reg']), 'operator' => $row['operator'], 'region' => $row['region'], 'status' => $row['status'], 'trash' => $row['trash'], 'group_id' => $row['group_id']);
	...
	function vew_product_price ($prod_id)
	{
	...
	function vew_points_all ($prod_id)
	{
