    ...
    <form action="?<?php htmlout($action); ?>" method="post">
	  ...
	  <div>
	    <label for="cash_in">ПОЛУЧЕНО: </label>
	     <input type="number"  name = "cash_in" id = "cash_in" min="0.00" max="100000" step="50.00"  value = "<?php if (isset($cash_in) and $cash_in != '')     htmlout($cash_in); ?>" />      
	  </div>
	<div>
	  <div>
	    <label for="cash_out">ВЫДАНО: </label>
	     <input type="number"  name = "cash_out" id = "cash_out" min="0.00" max="100000" step="50.00"  value = "<?php if (isset($cash_out) and $cash_out != '')     htmlout($cash_out); ?>" />      
	  </div>
	  <div>
